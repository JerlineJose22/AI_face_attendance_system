...
class Face_Recognizer:
    ...
    def get_face_database(self):
        if os.path.exists("data/features_all.csv"):
            ...
            for i in range(csv_rd.shape[0]):
                ...
                self.face_name_known_list.append(csv_rd.iloc[i][2])
                for j in range(3, csv_rd.shape[1]):
                    if csv_rd.iloc[i][j] == '':
            ...
            status_label.config(text=f"No data found, Please! Add Student and ", fg='red')
            return 0
    ...
    def draw_note(self, img_rd):
        ...
        cv2.putText(img_rd, "Making attendance for lecture 4", (20, 40), self.font, 1, (255, 255, 255), 1, cv2.LINE_AA)
    ...
    def run(self):
        ...
        def cam(x):
            ...
            """if the web cam where connected on multiple ports use the below codes"""
            ...
            #         selected_number = int(listbox.get(selected_index[0]))  # Get the selected number
            #         cap = cv2.VideoCapture(selected_number)              # Get video stream from camera
            #         self.process(cap)

            #         cap.release()
            #         cv2.destroyAllWindows()
            #     else:
            #         label_result.config(text="No number selected")
            ...
            # # Create a Listbox to display numbers with a scrollbar
            # scrollbar = tk.Scrollbar(root)
            # scrollbar.pack(side=tk.RIGHT, fill=tk.Y)

            # listbox = tk.Listbox(root, yscrollcommand=scrollbar.set)
            # for number in range( 101):  # Insert numbers from -100 to 100 (infinite list)
            #     listbox.insert(tk.END, str(number))  # Convert number to string for insertion
            # listbox.pack(side=tk.LEFT, fill=tk.BOTH)

            # scrollbar.config(command=listbox.yview)

            # # Bind the selection event to the get_selected_number function
            # listbox.bind("<ButtonRelease-1>", get_selected_number)
        ...
        def cctv():
            ...
            def capture():
                cap = cv2.VideoCapture(entry_box.get())# 'rtsp://example.com/media.mp4'  # RTSP, RTMP, HTTP stream
                self.process(cap)
            ...
            tk.Button(root, text="submit", command=capture,bd=10,font=("times new roman", 15),bg="black",fg="green",width=7).place(x=150,y=90)
            root.mainloop()

        window =tk.Toplevel()
        window.title("Camera Source")
        ...
        ri = Image.open("UI_Image/webcam.png")
        r = ImageTk.PhotoImage(ri)
        label1 = Label(window, image=r)
        label1.image = r
        label1.place(x=250, y=200)

        vi = Image.open("UI_Image/cctv cam.png")
        v = ImageTk.PhotoImage(vi)
        label3 = Label(window, image=v)
        label3.image = v
        label3.place(x=623, y=200)
