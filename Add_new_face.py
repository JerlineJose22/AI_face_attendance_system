...
class Face_Register:
        def __init__(self):
            ...
            self.input_rollno=tk.Entry(self.frame_right_info,width=25)
        ...
        def GUI_get_input_name(self):
            ...
            self.input_rollno_char=self.input_rollno.get()
        ...
        def GUI_info(self):
            ...
            tk.Label(self.frame_right_info,text="Roll Number").grid(row=9,column=0,sticky=tk.W,padx=5,pady=0)
            self.input_rollno.grid(row=9,column=1,padx=0,pady=2)

            tk.Button(self.frame_right_info,
                    text='Input',
                    command=self.GUI_get_input_name,bd=2).grid(row=9, column=2, padx=5)

            #  Save current face in frame
            tk.Label(self.frame_right_info,
                    font=self.font_step_title,
                    text="Save face image").grid(row=10, column=0, columnspan=2, sticky=tk.W, padx=5, pady=20)

            tk.Button(self.frame_right_info,
                    text='Save current face',
                    command=self.save_current_face,bd=3,bg="white",fg="black",font=tkFont.Font(family='Helvetica', size=11, weight='bold')).grid(row=12, column=0, columnspan=3, sticky=tk.W,)
            
            # submit
            tk.Button(self.frame_right_info,
                      text="Submit",
                      command=self.Submit,bd=2,font=tkFont.Font(family='Helvetica', size=15, weight='bold'),fg='black',bg='red').grid(row=12,column=2,columnspan=4,sticky=tk.W)
        ...
        def create_face_folder(self):
            ...
            if self.input_name_char:
                self.current_face_dir = self.path_photos_from_camera + \
                                        ...
                                        self.input_name_char+"_"+ self.input_rollno_char
            else:
                self.current_face_dir = self.path_photos_from_camera + \
                                        "person_" + str(self.existing_faces_cnt)
            self.current_face_dir=self.current_face_dir.upper()
        ...
        def Submit(self):
            ...
            self.log_all["text"] = "Student Added Successfully"
